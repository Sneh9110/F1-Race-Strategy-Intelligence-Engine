...
class LightGBMSafetyCarModel(BaseSafetyCarModel):
    ...
    def predict_deployment_window(self, prob: float, inp: PredictionInput):
        if prob > 0.75:
            return (inp.current_lap + 1, min(inp.total_laps, inp.current_lap + 6))
        if prob > 0.4:
            return (inp.current_lap + 2, min(inp.total_laps, inp.current_lap + 12))
        return None
